...
//               {/* Thêm khu vực chọn hướng giấy trước nút export */}
//               <div className="pdf-config-zone" style={{ margin: '20px 0', textAlign: 'center' }}>
//                 <label style={{ marginRight: '15px', fontWeight: 'bold' }}>Khổ giấy PDF: </label>
...
function App() {
  ...
  // Khởi tạo state định dạng hướng giấy: 'p' = dọc (portrait), 'l' = ngang (landscape)
  ...
  // ==========================================
  // HÀM XUẤT PDF CẬP NHẬT: FULL VIỀN KHÔNG CHỪA LỀ
  // ==========================================
  const exportToPDF = async () => {
    if (images.length === 0) return;

    // Khởi tạo PDF với hướng giấy được chọn từ state (orientation)
    const pdf = new jsPDF({
    ...
    // Lấy kích thước chính xác của trang giấy (thay đổi tùy theo dọc/ngang)
    const pdfWidth = pdf.internal.pageSize.getWidth();
    ...
    for (let index = 0; index < images.length; index++) {
      ...
      // Đọc ảnh và chuyển sang dữ liệu base64
      const dataUrl = await new Promise((resolve) => {
        const tempImg = new Image();
        tempImg.src = imgItem.url;
        tempImg.crossOrigin = "Anonymous";
      ...
      // Vẽ ảnh bắt đầu từ tọa độ (0,0) và kéo giãn vừa khít kích thước trang giấy
        ...
        <div className="container">
          ...
          {images.length > 0 && (
            <div className="preview-section">
              ...
              {/* KHU VỰC LỰA CHỌN KHỔ GIẤY DỌC / NGANG */}
              <div className="pdf-config-zone" style={{ margin: '25px 0', textAlign: 'center', backgroundColor: '#f8f9fa', padding: '12px', borderRadius: '8px', border: '1px dashed #ccc' }}>
                <span style={{ marginRight: '15px', fontWeight: 'bold', color: '#333' }}>Khổ giấy PDF:</span>
                <label style={{ marginRight: '20px', cursor: 'pointer', fontSize: '15px' }}>
                  <input
                    type="radio"
                    name="pdfOrientation"
                    value="p"
