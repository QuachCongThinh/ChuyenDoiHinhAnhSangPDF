        ...
        <div className="item-meta">
          ...
          <div className="item-actions-grid">
            ...
            <button
              className="btn-action-tile"
              onClick={() => rotateImageReal(img.id, img.url, 90)}
              title="Xoay xuôi chiều kim đồng hồ 90 độ"
            >
              🔄 Xoay 90°
            </button>
            <button
...
function App() {
  ...
  const rotateImageReal = (id, currentUrl, degrees = 90) => {
    ...
    img.onload = () => {
      ...
      // Khi xoay 90 hoặc -90 độ, chiều rộng và chiều cao sẽ hoán đổi cho nhau
            ...
            prev.map((item) =>
              item.id === id
                ? {
                  ...item,
                  url: rotatedUrl,
                  // Tính toán lại góc xoay hiển thị (luôn nằm trong khoảng 0-350)
                  rotation: ((item.rotation || 0) + degrees + 360) % 360
                }
                : item
            )
          );
